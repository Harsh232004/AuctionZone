...
const SectionHeading = ({ children, color = 'bg-blue-600', className = '' }: { children: React.ReactNode; color?: string; className?: string;}) => {
  return (
    <div className="text-center mb-12">
      <motion.h2 className={`text-3xl lg:text-4xl font-bold text-gray-900 inline-block relative pb-3 ${className}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.5 }}>
        {children}
        <motion.div className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-1 w-1/2 rounded-full ${color}`} initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.7, delay: 0.2, ease: "easeOut" }} />
      </motion.h2>
    </div>
  );
};
      ...
      <div className="relative">
        ...
        <div className="bg-gray-900 text-white w-64 md:w-72 p-6 shadow-lg rounded-l-lg">
          ...
          <ul className="space-y-4">
            <li><Link to="/register" className="flex items-center text-gray-300 hover:text-blue-400 transition-colors"><FaUser className="mr-3" /> Bidder Registration</Link></li>
            <li><Link to="/login" className="flex items-center text-gray-300 hover:text-blue-400 transition-colors"><FaBuilding className="mr-3" /> Company Login</Link></li>
            <li><Link to="/login" className="flex items-center text-gray-300 hover:text-blue-400 transition-colors"><FaArrowCircleRight className="mr-3" /> Bidder Login</Link></li>
          </ul>
        </div>
      </div>
    </div>
...
const Home: React.FC = () => {
  ...
  return (
    <main className="bg-gray-50">
          ...
          <div className="container mx-auto">
            ...
            <div className="relative w-full h-[450px] overflow-hidden bg-gradient-to-br from-blue-50 via-indigo-100 to-blue-200 rounded-2xl shadow-lg mb-16">
              <AnimatePresence mode="wait">
                <motion.div key={currentSlide} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.7, ease: "easeInOut" }} className="w-full h-full absolute top-0 left-0 flex items-center px-6 md:px-20">
                  <div className="w-full flex flex-col md:flex-row items-center justify-between gap-10">
                    <div className="w-full md:w-1/2 text-center md:text-left">
                      <motion.span initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="inline-block bg-blue-600 text-white px-4 py-1.5 rounded-full font-semibold text-xs mb-4 shadow-md">{slide.pill}</motion.span>
                      <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-4xl lg:text-5xl font-extrabold text-gray-900 leading-tight">{slide.title.split("\n").map((line, i) => (<span key={i}>{line}<br /></span>))}</motion.h1>
                      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}><Link to={slide.link} className="mt-6 inline-block bg-blue-600 text-white font-bold py-3 px-8 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-105">{slide.buttonText}</Link></motion.div>
                    </div>
                    <div className="hidden md:flex w-1/2 justify-center items-center">
                      {slide.image && <motion.img src={slide.image} alt="AuctionZone App" className="max-w-full max-h-[350px]" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3, type: "spring" }} />}
                      {slide.features && <div className="text-gray-700 space-y-4">{slide.features.map((feature, i) => (<motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.2 }} className="flex items-center text-lg font-medium bg-white/50 p-3 rounded-lg shadow-sm"><FaCheckCircle className="w-5 h-5 text-green-500 mr-3 flex-shrink-0" />{feature}</motion.div>))}</div>}
                    </div>
                  </div>
                </motion.div>
              </AnimatePresence>
              <button onClick={prevSlide} aria-label="Previous Slide" className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/70 backdrop-blur-sm rounded-full p-2 shadow-lg hover:bg-white transition z-10 text-gray-800"><FaArrowLeft /></button>
              <button onClick={nextSlide} aria-label="Next Slide" className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/70 backdrop-blur-sm rounded-full p-2 shadow-lg hover:bg-white transition z-10 text-gray-800"><FaArrowRight /></button>
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">{sliderContent.map((_, idx) => (<button key={idx} aria-label={`Go to slide ${idx + 1}`} onClick={() => setCurrentSlide(idx)} className={`w-2.5 h-2.5 rounded-full transition-colors ${currentSlide === idx ? "bg-blue-600" : "bg-white/80"}`} />))}</div>
            </div>
            ...
            <div id="upcoming">
              <SectionHeading color="bg-blue-500" className="font-serif">Upcoming Auctions</SectionHeading>
              <div className="relative">
      ...
      <motion.section id="trending" className="py-16 bg-gray-900" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={sectionVariants}>
        <div className="container mx-auto">
          <SectionHeading color="bg-purple-500" className="text-white">Trending Now</SectionHeading>
          <div className="relative h-[450px] md:h-[550px] w-full rounded-2xl overflow-hidden shadow-2xl">
            <AnimatePresence>
              <motion.div key={currentTrendingSlide} className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${trendingItems[currentTrendingSlide].image})` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.2, ease: "easeInOut" }}>
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                <div className="absolute bottom-0 left-0 p-6 text-white">
                  <h3 className="text-2xl font-bold">{trendingItems[currentTrendingSlide].title}</h3>
                  <p className="mt-1 text-base">Current Bid: <span className="font-bold">${trendingItems[currentTrendingSlide].currentBid?.toLocaleString()}</span></p>
                </div>
              </motion.div>
            </AnimatePresence>
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
              {trendingItems.map((_, index) => (<button key={index} onClick={() => setCurrentTrendingSlide(index)} className={`w-2.5 h-2.5 rounded-full transition-colors duration-300 ${currentTrendingSlide === index ? 'bg-white' : 'bg-white/50'}`} aria-label={`Go to slide ${index + 1}`} />))}
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <motion.section className="py-16 bg-white" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={sectionVariants}>
        <div className="container mx-auto">
          <SectionHeading color="bg-pink-500" className="font-serif italic">From Our Blog</SectionHeading>
