...
const PlaceBidPage = () => {
  ...
  const { addItem } = useContext(CartContext); // Use the context
  const item = allAuctionItems.find((i) => i.id === Number(id));

  const [currentBid, setCurrentBid] = useState(item?.currentBid || item?.startingBid || 0);
  ...
  if (!item) {
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 text-center">
            <h2 className="text-3xl font-bold text-red-600 mb-4">Auction Item Not Found</h2>
            <p className="text-gray-700 mb-8">The item you are looking for might have expired or does not exist.</p>
  ...
  const handlePlaceBid = () => {
    if (bidAmount <= currentBid) {
      setMessage({ text: 'Your bid must be higher than the current bid.', type: 'error' });
      return;
    }
    setCurrentBid(bidAmount);
    setBidderCount(bidderCount + 1);
    setMessage({ text: `Success! You are now the highest bidder with $${bidAmount.toLocaleString()}.`, type: 'success' });
  };
  
  // New handler for adding items to the watchlist
  const handleAddToWatchlist = () => {
    if(item) {
        addItem(item);
        setMessage({ text: `"${item.title}" was added to your watchlist.`, type: 'success' });
    }
  };
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mb-6">Sold by: <span className="font-semibold text-blue-600">{item.sellerName}</span></p>
              ...
              <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg mb-8">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-3xl font-bold text-gray-900">${currentBid.toLocaleString()}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-sm text-gray-600">Ends In</p>
                        <p className="text-lg font-semibold text-red-600">{item.endsIn}</p>
                    </div>
                </div>
              ...
              <div className="space-y-4">
                <label htmlFor="bidAmount" className="block font-semibold text-gray-800">Your bid amount</label>
                <div className="flex items-center">
                    <span className="inline-flex items-center px-3 text-gray-500 bg-gray-100 border border-r-0 border-gray-300 rounded-l-md h-12">$</span>
                    <input 
                        type="number" 
                        id="bidAmount"
                        value={bidAmount}
                ...
                <div className="grid grid-cols-2 gap-4">
                    <motion.button 
                        ...
                        className="w-full flex items-center justify-center bg-blue-600 text-white font-bold py-3 px-6 rounded-lg shadow-md hover:bg-blue-700 transition-colors"
                    ...
                    <motion.button 
                        onClick={handleAddToWatchlist} // Connect the new handler
